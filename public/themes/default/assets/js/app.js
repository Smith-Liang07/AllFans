...
$(function () {
  ...
  $('#link_other a').attr('target', '_blank');
    ...
    $('body').on('click','.follow-user',function(e){
        ...
        if ($('.follow-user').data('price') == "0.00") {
            follow_btn = $(this).closest('.follow-links');
            ...
            $.post(SP_source() + 'checkout/config/' + $('.follow-user').data('timeline-id'), {}, function(data) {
    ...
    let createCheckoutSession = function() {
        console.log("timeline_id: " + $('.follow-user').data('timeline-id'));
        $.post(SP_source() + 'checkout/create-checkout-session/' + $('.follow-user').data('timeline-id'), function(data) {
            stripe
                .redirectToCheckout({
